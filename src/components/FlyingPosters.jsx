...
class Media {
  ...
  createShader() {
    ...
    img.onload = () => {
      texture.image = img;
      this.program.uniforms.uImageSize.value = [img.naturalWidth || 1, img.naturalHeight || 1];
    };
  }
...
export default function FlyingPosters({
  ...
  return (
    <div ref={containerRef} className={`relative w-full h-full overflow-hidden ${className}`} {...props}>
      <canvas ref={canvasRef} className="absolute top-0 left-0 w-full h-full block outline-none touch-none" />
    </div>
  );
}